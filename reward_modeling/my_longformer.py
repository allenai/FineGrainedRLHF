...
Note: Adding support for bf16 training.
"""
...
def tril_bfloat16(input_tensor, diagonal=0):
    if input_tensor.dtype == torch.bfloat16:
        row_indices, col_indices = torch.meshgrid(torch.arange(input_tensor.size(0)), torch.arange(input_tensor.size(1)), indexing='ij')
        mask = (col_indices - row_indices <= diagonal).to(input_tensor.device)
        tril_tensor = torch.where(mask, input_tensor, torch.zeros_like(input_tensor))
        return tril_tensor
    else:
...
class LongformerSelfAttention(nn.Module):
    ...
    def _mask_invalid_locations(input_tensor, affected_seq_len) -> torch.Tensor:
        ...
        # GPT-4 solution
        beginning_mask_2d = tril_bfloat16(input_tensor.new_ones(affected_seq_len, affected_seq_len + 1)).flip(dims=[0])
