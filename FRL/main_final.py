...
class PPOTrainer:
    def __init__(self,
        ...
        if self.args.mode == 'train':
            if not args.nolog and accelerator.is_main_process:
                wandb.init(entity="uwnlp", project='asqa_RL', name=args.run_name, config=args)
    ...
    def valid(self, step):
        ...
        if not self.args.nosave and accelerator.is_main_process:
            ...
            if prev_best_step is None or reward_normalizeds > self.eval_accs[prev_best_step]:
                if prev_best_step is not None:
                    try:
                        os.remove(f'{self.args.model_dir}/ckp_{prev_best_step}_policy.pth')
                        os.remove(f'{self.args.model_dir}/ckp_{prev_best_step}.pth')
                    except:
                        log.warning(f'Cannot remove previous best ckpt!')
                shutil.copy(f'{self.args.model_dir}/last_policy.pth', f'{self.args.model_dir}/ckp_{step}_policy.pth')       
                ...
                accelerator.save(policy_model_state_dict, f'{self.args.model_dir}/best_policy.pth')
    ...
    def save(self, step):
        ...
        if not self.args.policy_value_sharing:
            ...
            value_linear_state_dict = accelerator.unwrap_model(self.value_model.linear).state_dict()
        ...
        accelerator.wait_for_everyone()
        accelerator.save(policy_model_state_dict, f'{self.args.model_dir}/last_policy.pth')
...
def main():
    ...
    if not args.nosave:
        if args.mode == 'train':
            ...
            args.generated_dir = os.path.join(args.save_dir, 'knowledge')
            if accelerator.is_main_process:
                for d in [args.save_dir, args.generated_dir]:
                    ensure_dir(d)
